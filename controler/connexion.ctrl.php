<?php
  $config = parse_ini_file('../config/config.ini');                           // Recupération de données de configuration.
  require_once('../model/DAOClass.class.php');
  require_once('../framework/view.class.php');

  $bdd = new DAOClass($config['database_path']);
  if (isset($_POST['connexion'])) {                                           // On test si la page à déjà été remplie
    ...
    if (!empty($login_valide)) {                                              // test si le pseudo existe
      ...
      if (password_verify($_POST['mdp'] , $pass_bdd['mdp'])) {                // On compare le mot de passe de la base de données et celui entré par l'utilisateur
        ...
        $view->region = $region;
        $view->show();
      } else {                                                                //si le mot de passe est différent on renvoi l'utilisateur vers la même page en mettant à jour l'erreur
    ...
    } else {                                                                  //si le pseudo n'existe pas on le renvoi vers la même page en mettant à jour l'erreur
      $view = new View('connexion.view.php');
      $view->erreur = "Identifiant non reconnu !";
      $view -> show();
    }
  } else{                                                                     //si la page n'a pas encore été rempli on l'affiche sans rien faire
    $view = new View('connexion.view.php');
    $view -> show();
  }
 ?>
